...
from eval_util_petopia import result_dict_for_batched_example
...
def eager_eval_loop(
        ...
        custom_list=None,
        ckpt_id=None,
        eval_metrics_dir=None,
        custom_map_list=None):
    ...
    def compute_eval_dict(features, labels):
        ...
        # print(f'1detection_classes: {str(detections["detection_classes"][0].astype(np.uint8))}')
        # 'detection_classes': <tf.Tensor 'Postprocessor/BatchMultiClassNonMaxSuppression/stack_8:0' shape=(1, 100) dtype=float32>,
        class_agnostic = (
    ...
    def convert_custom_class(eval_dict, custom_classes):
        groundtruth_classes = eval_dict["groundtruth_classes"][0].numpy()
        detection_classes = eval_dict["detection_classes"][0].numpy()
        # if 8 in groundtruth_classes or 9 in groundtruth_classes:
        #     print(f'groundtruth_classes : {str(groundtruth_classes)}')
        #     print(f'1detection_classes : {str(detection_classes)}')

        for gc in groundtruth_classes:
            if gc in per_class_categories.keys() \
                    and per_class_categories[gc]['id'] in custom_classes.keys() \
                    and len(custom_classes[per_class_categories[gc]['id']]) > 1:
                before_id_list = custom_classes[per_class_categories[gc]['id']]
                after_id = gc

                for before in before_id_list:
                    for i, dc in enumerate(detection_classes):
                        if dc == before:
                            detection_classes[i] = after_id

        eval_dict["detection_classes"] = tf.constant([detection_classes], dtype=tf.int64)
        # if 8 in groundtruth_classes or 9 in groundtruth_classes:
        #     print(f'2detection_classes : {str(eval_dict["detection_classes"][0].numpy())}')
        return eval_dict
    ...
    for k, v in per_class_categories.items():
        label_arr[v['name']] = v['id']

    for before_class_names, after_class_names in custom_list.items():
        if before_class_names in label_arr:
            custom_classes[label_arr[before_class_names]] = []
            for a in after_class_names:
                if a in label_arr:
                    custom_classes[label_arr[before_class_names]].append(label_arr[a])

    # print(f'custom_classes : {str(custom_classes)}')
    for i, (features, labels) in enumerate(eval_dataset):
        eval_dict, losses_dict, class_agnostic = compute_eval_dict(features, labels)
        eval_dict = convert_custom_class(eval_dict, custom_classes)
    ...
    file_lines = ['CUSTOM/mAP: ']
    custom_ap = []

    with open(os.path.join(eval_metrics_dir, f'{ckpt_id}_result'), 'w') as f:
        for k in eval_metrics:
            if str(k) != 'PascalBoxes_Precision/mAP@0.5IOU':
                tf.compat.v2.summary.scalar(k, eval_metrics[k], step=global_step)

            if 'AP' in str(k):
                log_str = f'{str(k)}: {str(float(eval_metrics[k]) * 100)}'
            else:
                log_str = f'{str(k)}: {str(float(eval_metrics[k]))}'
            if 'PascalBoxes_PerformanceByCategory' in log_str:
                class_name = log_str.split('/')[2].split(':')[0]
                ap = float(log_str.split('/')[2].split(':')[1])
                if class_name in custom_map_list:
                    custom_ap.append(ap)

            tf.logging.info(log_str)
            file_lines.append(log_str)

        tf.compat.v2.summary.scalar('PascalBoxes_Precision/mAP@0.5IOU', np.mean(custom_ap)/100, step=global_step)
        tf.logging.info(str(custom_ap))
        file_lines[0] = file_lines[0] + str(np.mean(custom_ap))
        f.write("\n".join(file_lines))
...
def eval_continuously(
    ...
    tf.logging.info(f'checkpoint path : {checkpoint_dir}')
    ckpt = tf.compat.v2.train.Checkpoint(
        step=global_step, model=detection_model)

    ckpt.restore(checkpoint_dir).expect_partial()
    ...
    for eval_name, eval_input in eval_inputs:
        ...
        with summary_writer.as_default():
            eager_eval_loop(
                ...
                ckpt_id=os.path.basename(checkpoint_dir),
